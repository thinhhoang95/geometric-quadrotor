...
for i=1:(window_size-1)
    ...
    remaining_residual_length = residual_vector_length - length(rowJ);
    ...
    Jpr = Ro(:,:,i)'*(Po(i+1,:)-Po(i,:)-Vo(i,:)*imu_sampling_time*kf_interval_imu-0.5*[0 0 9.81]*((imu_sampling_time*kf_interval_imu).^2))'; % Jacobian wrt phi1
    rowJ = [rowJ Jpr];
    Jvv = -Ro(:,:,i)'*imu_sampling_time*kf_interval_imu; % Jacobian wrt v1
    rowJ = [rowJ Jvv]; 
    Jvp = -zeros(3); % Jacobian wrt p1
    rowJ = [rowJ Jvp];
    rowJ = [rowJ zeros(3) zeros(3)]; % Rij wrt phij and vj
    rowJ = [rowJ Ro(:,:,i)'*Ro(:,:,i+1)]; % Jacobian wrt phi2
    % For the rest of the residuals, put in 0
    remaining_residual_length = residual_vector_length - length(rowJ);
    for j=1:remaining_residual_length
        rowJ = [rowJ zeros(3,1)];
    ...
    Gamma = focal_length * Ro(:,:,i)' * (Lo - Po(i,:)');
    xp = Gamma(1); yp = Gamma(2); zp = Gamma(3);
    dGdP = focal_length * hat(Ro(:,:,i)' * (Lo - Po(i,:)'));
    addToRowJ = [(dGdP(1,1)*zp - dGdP(3,1)*xp)/zp^2 (dGdP(1,2)*zp - dGdP(3,2)*xp)/zp^2 (dGdP(1,3)*zp - dGdP(3,3)*xp)/zp^2;
    ...
    dGdPos = -focal_length*eye(3);
    addToRowJ = [(dGdPos(1,1)*zp - dGdPos(3,1)*xp)/zp^2 (dGdPos(1,2)*zp - dGdPos(3,2)*xp)/zp^2 (dGdPos(1,3)*zp - dGdPos(3,3)*xp)/zp^2;
    ...
    for j=length(rowJ):(residual_vector_length-4)
        rowJ = [rowJ zeros(2,1)];
    end
    % Jacobian wrt landmark position
    dGdPos = -focal_length*Ro(:,:,i)';
    addToRowJ = [(dGdPos(1,1)*zp - dGdPos(3,1)*xp)/zp^2 (dGdPos(1,2)*zp - dGdPos(3,2)*xp)/zp^2 (dGdPos(1,3)*zp - dGdPos(3,3)*xp)/zp^2;
        (dGdPos(2,1)*zp - dGdPos(3,1)*yp)/zp^2 (dGdPos(2,2)*zp - dGdPos(3,2)*yp)/zp^2 (dGdPos(2,3)*zp - dGdPos(3,3)*yp)/zp^2];
    rowJ = [rowJ addToRowJ];
    J = [J; rowJ];
